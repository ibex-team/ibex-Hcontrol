...
//			     std::cout << *y_cell << std::endl;
	...
	if (!top1_minmax)
		ibex_error("[ibex_EvalMax] -- null ptr in eval: top1 element has no BxpMinMaxSub.");
	double new_fmax_ub = top1_minmax->maxfxy.ub(); // get the upper bound of max f(x,y_heap)
	...
	double new_fmax_lb = top2_minmax->maxfxy.lb(); // get the lower bound of max f(x,y_heap)
...
bool EvalMax::handle_cell(const IntervalVector& x_box, BxpMinMax* data_x , Cell* y_cell, double loup, bool no_stack) {
	...
	if (loup < best_lb) {  // midres.lb()> best_max ->is now false since fmax.ub() unchanged in
...
bool EvalMax::stop_crit_reached(int current_iter, const BxpMinMax& data_x) const {
	...
	////            std::cout<<"Stop light solver: list reached max elem"<<std::endl;
	//		return true;
	//        }
...
//		//            std::cout<<"Stop light solver: Csp case, upper bound lower than 0"<<std::endl;
//		return true;
//	}
...
//		// std::cout<<"             box: "<< xy_box<<" random start: "<<xy_rand<<std::endl;
//		local_solver->minimize(xy_rand,final_point,1e-3); // maximizes
//		// std::cout<<"             local solution: "<<xy_max<<std::endl<<"     goal value: "<<eval_all(xy_sys.goal,xy_max)<<std::endl;
//
//		// update current y box in the xy_box for evaluation at a point y given by local optim
//		for (int k =x_box.size(); k<xy_box.size(); k++)
//			xy_box_eval[k] = final_point[k];
//		// y_max must respect constraints
//		if (check_constraints(xy_box) == 2) 	{
//			// eval objective function at (x_box,y_max)
//			Interval eval = xy_sys.goal->eval(xy_box_eval);
//			//            std::cout<<"local optim solution: "<<xy_box_eval<<" objectif eval at solution: "<<eval<<std::endl;
...
//	//    std::cout<<"natural eval: "<<eval<<std::endl;
//	//    Interval eval_centered =  f->eval_centered(box);
//	//    std::cout<<"centered eval: "<<eval_centered<<std::endl;
//	//    Interval eval_baumann = f->eval_baumann(box);
//	//    std::cout<<"baumann eval: "<<eval_baumann<<std::endl;
//	//    eval &= eval_centered;
//	//    eval &= eval_baumann;
//	//    std::cout<<"eval intersect: "<<eval<<std::endl;
//	//    return eval_baumann;
